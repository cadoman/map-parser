...
class PolygonGroup:
    ...
    def filter_polygons(self, min_area:int):
        ...
        '''
        big_polygons = [pol for pol in self.polygon_list if pol.area >= min_area]
    ...
    def without_polygons_contained_in_other_group(self, other_group):
        ...
        '''
        not_contained = [pol for pol in self.polygon_list if not is_contained(pol, other_group.polygon_list)]
        return PolygonGroup(not_contained, self.name)
    def display(self):
        points = np.concatenate(([pol.exterior.coords for pol in self.polygon_list])).astype(int)
